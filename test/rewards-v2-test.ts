...
describe("CometRewards", () => {
  ...
  describe.only("setRewardConfig", () => {
    ...
    it("does not allow anyone but governor to set config", async () => {
      ...
      await expect(
        rewards.connect(alice).setRewardConfig(comet.address, COMP.address)
        //).to.be.revertedWith(`custom error 'NotPermitted("${alice.address}")'`);
      ).to.be.revertedWith(`custom error 'NotPermitted(address)'`);
    });
  });

  describe.only("withdrawToken", () => {
  ...
  describe.only("setRewardsClaimed", () => {
  ...
  describe.only("transferGovernor", () => {
